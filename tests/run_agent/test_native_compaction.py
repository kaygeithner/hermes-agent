...
class TestInterimMerge:
    def test_prior_checkpoint_survives_new_reasoning(self):
        from agent.native_compaction import merge_interim_reasoning_items

        checkpoint = {"type": "compaction", "encrypted_content": "old"}
        new_reasoning = {"type": "reasoning", "encrypted_content": "new"}
        assert merge_interim_reasoning_items(
            [checkpoint, {"type": "reasoning", "encrypted_content": "stale"}],
            [new_reasoning],
        ) == [checkpoint, new_reasoning]

    def test_new_checkpoint_replaces_old_checkpoint(self):
        from agent.native_compaction import merge_interim_reasoning_items

        old = {"type": "compaction", "encrypted_content": "old"}
        new = {"type": "compaction", "encrypted_content": "new"}
        assert merge_interim_reasoning_items([old], [new]) == [new]

    def test_non_lists_are_safe(self):
        from agent.native_compaction import merge_interim_reasoning_items
