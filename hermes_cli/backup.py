...
def _should_exclude(rel_path: Path) -> bool:
    ...
    # A full default-profile backup also contains named profiles. Apply the
    # generated-root exclusions relative to each profile home, not only to the
    # top-level HERMES_HOME.
    logical_parts = parts[2:] if len(parts) >= 2 and parts[0] == "profiles" else parts
    if any(
        logical_parts[: len(prefix)] == prefix
...
_QUICK_STATE_FILES = (
    ...
    "cron/jobs.json",
    "cron/executions.db",
    "cron/notepad.db",                   # per-job durable scratchpad state
    "gateway_state.json",
